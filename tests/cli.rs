...
fn ensure_test_bins() {
    INIT.call_once(|| {
        let status = Command::new("make")
            .arg("test-bins")
            .status()
            .expect("Failed to run 'make test-bins'");
        assert!(status.success(), "make test-bins failed");
    });
}
...
fn run_case(path: &Path) -> datatest::Result<()> {
    ...
    if !exe.exists() {
        panic!(
            "Binary {} not found. Make sure 'make test-bins' succeeded.",
            exe.display()
        );
    }
...
datatest::harness! {
    { test = run_case, root = "./test-files", pattern = r#"^.*\.(c|S)$"# },
}
